...
Heightmap HeightmapGenerator::Generate(int width, int height){
	...
	diamond(heightmap, points, width-1, height-1, width, height);
	}

void Heightmap::diamond(Heightmap heightmap, Point points, int sizex, int sizey, int width, int height){
	int hsx = sizex / 2;
	int hsy = sizey / 2;
	...
	for (int i = 0; i < (sizeof(points)/sizeof(points[0])); i++)
	{
		...
		gen4valdia(a,b,c,d,hsx,hsy,valuea,valueb,valuec,valued);
...
void Heightmap::square(Heightmap heightmap, Point points, int sizex, int sizey, int width, int height){
	...
	for (int i = 0; i < points.Length; i++)
	{
		...
		// generate the 4 values of colour the points will have
		int valuea, valueb, valuec, valued;
		gen4valsqr(a,b,c,d,hsx,hsy,valuea,valueb,valuec,valued);
...
void gen4valdia (Point a, Point b, Point c, Point d, 
              int hsx, int hsy, 
...
void gen4valsqr (Point a, Point b, Point c, Point d, 
              int hsx, int hsy, 
			  int& valuea, int& valueb, int& valuec, int& valued){
	valuea = myavg(
